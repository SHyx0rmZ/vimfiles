...
syn keyword        glslStorageClass        smooth flat noperspective
...
" swizzling
syn match          glslSwizzle             /\.[xyzw]\{1,4\}\>/ms=s+1
syn match          glslSwizzle             /\.[rgba]\{1,4\}\>/ms=s+1
syn match          glslSwizzle             /\.[stpq]\{1,4\}\>/ms=s+1
...
if version >= 508 || !exists("did_glsl_syn_inits")
  ...
  HiLink glslState xGreen
  HiLink glslType xGray
  HiLink glslSwizzle xRed
  HiLink glslStorageClass xGray
  HiLink glslStatement xYellow
  HiLink glslConditional Comment
